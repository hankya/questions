...
class Window(gtk.Window):
    ...
    def _get_view_image(self, x, y, width, height):
        root = self.output.get_parent_window()
        rect = self.output.get_allocation()
        
        print 'x is %s, y is %s' % (rect.x, rect.y)
        '''
        x = rect.x
        y = rect.y
        width = rect.width
        ...
        pixbuf.get_from_drawable(root, root.get_colormap(), x, y, 0, 0, width, height)
        print x, y, width, height
        return pixbuf

    def take_screenshot(self, x, y, width, height, path=None):
        if path is None:
            path = time.strftime("%m%d%y-%H%M%S.png")

        pixbuf = self._get_view_image(x, y, width, height)
