...
class JyeooSpider(CrawlSpider):
    ...
    start_urls = ['http://www.jyeoo.com/math',] 
    ...
    rules = (
        ...
        Rule(SgmlLinkExtractor(allow=('http://www.jyeoo.com/\w+/ques/detail/[A-Za-z0-9]{8}-[A-Za-z0-9]{4}-[A-Za-z0-9]{4}-[A-Za-z0-9]{4}-[A-Za-z0-9]{12}',)), callback='parse_item', process_request=add_meta),
        )     
    ...
    def parse_item(self, response):
        ...
        hxs= HtmlXPathSelector(response)
        item = QuestionItem()
        item['image_urls'] = hxs.select('//img/@src').extract()
